...
const loadCalendarData = useCallback(async () => {
  ...
  try {
    ...
    console.log('📅 All school events:', events); // Debug
    console.log('👦 Selected child class:', selectedChild.class_name); // Debug
    
    // ✅ FIX: Filter events that apply to this child's class
    const relevantEvents = events?.filter(event => {
      // Check if affects_classes is an array and contains "All" or the child's class
      if (!event.affects_classes) return false;
      
      const affectedClasses = Array.isArray(event.affects_classes) 
        ? event.affects_classes 
        : JSON.parse(event.affects_classes);
      
      // Check if it's for all students or specifically for this class
      return affectedClasses.includes('All') || 
             affectedClasses.includes(selectedChild.class_name) ||
             affectedClasses.includes('Primary') && selectedChild.class_name.startsWith('Y') && parseInt(selectedChild.class_name.slice(1)) <= 6 ||
             affectedClasses.includes('Lower Secondary') && selectedChild.class_name.startsWith('Y') && parseInt(selectedChild.class_name.slice(1)) >= 7;
    }) || [];
    
    console.log('✅ Filtered events for this child:', relevantEvents); // Debug
    ...
    console.log('📝 Tests for this class:', tests); // Debug
    ...
    console.log('📚 Homework for this class:', homework); // Debug
