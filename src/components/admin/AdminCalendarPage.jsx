    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="relative">
          ...
          <Shield className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-emerald-600" size={24} />
        </div>
      </div>
    );
  }
            ...
            <div>
              <div className="flex items-center gap-3 mb-2">
                ...
                <div>
                  ...
                  <p className="text-emerald-100 text-sm">Manage events, breaks & view all tests</p>
                </div>
              </div>
            </div>
            ...
            {daysInMonth.map((date, idx) => {
              ...
              return (
                <div
                  ...
                  className={'min-h-[100px] rounded-xl border-2 p-2 transition-all relative group cursor-pointer ' + 
                    (hasBreak ? 'bg-gray-100 border-gray-300' :
                     today ? 'border-emerald-500 bg-emerald-50 hover:shadow-lg' :
                     'border-gray-200 hover:border-emerald-300 hover:shadow-md')}
                >
                  <span className={'text-sm font-bold mb-1 block ' + 
                    (today ? 'text-emerald-600' : hasBreak ? 'text-gray-400' : 'text-gray-700')}>
                    {dayNumber}
                  </span>
                  ...
                  <div className="space-y-1">
                    ...
                    {tests.map((test, i) => (
                      <div 
                        ...
                        title={'Teacher: ' + (test.teachers?.full_name || 'Unknown')}
                      >
                        ...
                        {events.map((event, i) => (
                          <div key={'tooltip-event-' + i} className="mb-2">
                            ...
                            {event.scope === 'specific' && (
                              <div className="text-yellow-300 text-xs">Classes: {event.affected_classes?.join(', ')}</div>
                            )}
                          </div>
                        ))}
                        {tests.map((test, i) => (
                          <div key={'tooltip-test-' + i} className="mb-1">
                            ...
                            <div className="text-gray-400 text-xs">Teacher: {test.teachers?.full_name || 'Unknown'}</div>
                          </div>
                        ))}
                      </div>
                    </div>
            ...
            <div className="bg-gradient-to-r from-emerald-600 to-teal-600 px-6 py-4 flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-emerald-100 text-sm">Manage school calendar</p>
                </div>
              </div>
              <button 
                onClick={() => {
                ...
                <div className="space-y-2">
                  <label className="flex items-center gap-3 p-3 border border-gray-300 rounded-xl cursor-pointer hover:bg-gray-50">
                    <input
                      ...
                      onChange={(e) => setFormData({ ...formData, scope: e.target.value, affected_classes: [] })}
                      className="w-4 h-4 text-emerald-600"
                    />
                    <div>
                      <p className="font-medium text-gray-900">All School</p>
                      <p className="text-xs text-gray-600">Applies to all classes</p>
                    </div>
                  </label>
                  ...
                  <label className="flex items-center gap-3 p-3 border border-gray-300 rounded-xl cursor-pointer hover:bg-gray-50">
                    ...
                    />
                    <div>
                      <p className="font-medium text-gray-900">Specific Classes</p>
                      <p className="text-xs text-gray-600">Select which classes this affects</p>
                    </div>
                  </label>
                </div>
              </div>
                    ...
                    {availableClasses.map(className => (
                      <label
                        ...
                        className={'flex items-center gap-2 p-2 rounded-lg cursor-pointer transition-colors ' +
                          (formData.affected_classes.includes(className) ? 'bg-emerald-100 border border-emerald-300' : 'bg-gray-50 hover:bg-gray-100')}
                      >
                        <input
                          type="checkbox"
