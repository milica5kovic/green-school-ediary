...
const HomeworkCard = ({ classId, homework, onEdit, onDelete }) => {
  ...
  const isDueToday = hw.isDueToday();
  const daysUntilDue = hw.getDaysUntilDue();

  // Extract class info from classId
  const className = classId.split('-')[0];
  const classTime = classId.split('-')[1];
      ...
      <div className="flex justify-between items-start">
        <div className="flex-1">
          ...
          <div className="bg-white rounded-lg p-4 mb-4 border border-gray-200">
            <p className="text-gray-800 leading-relaxed">{hw.description}</p>
          </div>
          ...
          {hw.hasAttachments() && (
            <div className="mb-4">
              <p className="text-sm font-semibold text-gray-700 mb-2">
                📎 Attachments ({hw.getAttachmentCount()})
              </p>
