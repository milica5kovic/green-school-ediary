  ...
  <div className="space-y-6">
                ...
                <>
                  <div className="flex items-center gap-3">
                    <span className={`font-medium ${
                      subject.is_active ? 'text-gray-800' : 'text-gray-500 line-through'
                    }`}>
                    ...
                    <button
                      onClick={async () => {
                        try {
                          ...
                          setSubjectSuccess(
                            subject.is_active 
                              ? 'Subject deactivated! It will no longer appear in dropdowns.' 
                              : 'Subject activated! It will now appear in dropdowns.'
                          );
                          setTimeout(() => setSubjectSuccess(''), 3000);
                        } catch (err) {
                          console.error('Error toggling subject:', err);
                          setSubjectError('Failed to update subject status');
    ...
    <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
      <p className="text-sm text-yellow-900">
        <strong>💡 Tip:</strong> Instead of deleting subjects, consider <strong>deactivating</strong> them (🔴) to preserve historical data while hiding them from teacher assignment dropdowns. Inactive subjects can be reactivated (🟢) at any time.
      </p>
    </div>
  </div>
);
